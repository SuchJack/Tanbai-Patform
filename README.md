...
直接在后端复制一份application-local.yml.temp，改名为application-local.yml
...
- [x] 1. 优化海报制作响应，从秒级提高至毫秒级（不考虑网络延迟）

- [ ] 2. 新增自定义海报（参考腾讯活码的节日海报）

- [x] 3. 支付功能在电脑端小程序存在BUG：无法触发订单的状态检测API，无法修改问题状态为`is_paid`

  - - [ ] 优化：订单表添加业务冗余字段，设置定时任务以识别系统处理失败的订单

  - - [x] 修复：查找电脑端支付后二维码状态、回调等API，从根源上修复BUG
  
    - 使用原生`wx.requestPayment`发送支付请求


- [x] 3. 数据库字段优化，改为驼峰命名，设置逻辑删除以追溯用户不良信息
- [ ] 4. 增强活动营销，新增管理员群发订阅消息
- [x] 5. 修复冷启动时进入问题详情页加载失败（可能与鉴权session策略有关），做异常兜底
     - 鉴权由`jwt`改为`Sa-token`，拓展性、安全性、可维护性更强
