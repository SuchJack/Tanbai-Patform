...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public Question createQuestion(QuestionDTO questionDTO) {
        ...
        if (!weChatUtils.verifyContent(content)) {
            throw new BusinessException("内容非法");
        }
        // 内容检查通过，继续创建问题
    ...
    @Override
    public byte[] generateQRCodeByQuestionId(Long questionId, HttpServletResponse response) {
        try {
            // 使用 WeChatUtils 生成小程序码
            return weChatUtils.generateWxaCode(
                    questionId.toString(),
