...
// 获取 code 登录凭证
onShow(() => handleCodeAndLogin(true))
...
const handleUserInfoUpdate = async (info: { avatarUrl: string; nickName: string }) => {
  ...
  try {
    ...
    // 先发送到后端
    const res: any = await updateUserProfileAPI(updateData)
    ...
    try {
      ...
      if (userInfoRes.code === 200 && userInfoRes.data) {
        ...
        console.log('用户信息已更新:', userInfoRes.data)
      }
    } catch (fetchErr) {
      console.error('获取最新用户信息失败:', fetchErr)
      // 即使获取最新信息失败，也使用已知的新信息更新 store
