<template>
  <view class="question-detail" :class="{ 'dark-mode': false }">
    ...
    <!-- 登录错误时显示重试按钮 -->
    <view v-else-if="loginError" class="login-error-container">
      <text class="error-text">登录失败，请重试</text>
    ...
    <view v-else>
        ...
        <view v-else class="answer-list">
          <view v-for="answer in answers" :key="answer.answer.id" class="answer-item">
            ...
            <view class="answer-content-wrapper">
              ...
              <view
                v-if="
                  answer.answer.userId === userStore.profile?.userId ||
                  question.creatorId === userStore.profile?.userId
                "
                class="delete-btn"
            ...
            <view class="replies" v-if="answer.replies && answer.replies.length > 0">
              ...
              <view v-for="reply in answer.replies" :key="reply.id" class="reply-item">
                ...
                <view v-else class="reply-header">
                  ...
                  <view
                    v-if="
                      reply.userId === userStore.profile?.userId ||
                      question.creatorId === userStore.profile?.userId
                    "
                    class="delete-btn"
            ...
            <view
              ...
              v-if="userStore.profile?.userId === question.creatorId"
            >
      ...
      <view class="bottom-buttons">
        <!-- 创建者本人可见的按钮组 -->
        <template v-if="userStore.profile?.userId === question.creatorId">
...
const retryLogin = async () => {
  ...
  const questionId = currentPage.options.id || scanQuestionId.value

  if (questionId) {
    await getQuestionDetail(String(questionId))
  } else {
    loading.value = false
...
// 获取问题详情
const getQuestionDetail: any = async (questionId: string) => {
  loading.value = true
  try {
    // 等待登录完成
    await waitForLogin()
    ...
    if (res.code === 200) {
      ...
      // 如果返回未登录，尝试重新登录
      const loginSuccess = await handleCodeAndLogin(false)
      if (loginSuccess) {
        // 登录成功后重新获取问题详情
...
const handleScanConfirm = async () => {
  try {
    await getQuestionDetail(String(scanQuestionId.value))
  } catch (err) {
    console.error('获取问题详情失败:', err)
    loginError.value = true
    uni.showToast({
...
const showCommentModal = async () => {
  ...
  // 检查模板消息订阅权限
  try {
    const subscribeResult: any = await uni.requestSubscribeMessage({
      tmplIds: MESSAGE_TMP_IDS, // 替换为您的模板消息ID
    })
...
const submitReply = async () => {
  ...
  try {
    const res: any = await replyCommentAPI({
...
const confirmDeleteAnswer = (answerId: number) => {
  // 判断是否为问题创建者
  const isCreator = question.value.creatorId === userStore.profile?.userId
...
const confirmDeleteReply = (replyId: number) => {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          const res: any = await deleteReplyAPI({ replyId, userStore, isCreator })
          ...
          if (orderRes.code === 200) {
            ...
            if (payRes.code === 200) {
              ...
              wx.requestPayment({
                ...
                success: async () => {
                  // 查询订单状态
                  const statusRes: any = await getOrderStatusAPI({
                    orderNumber,
                    questionId: question.value.id,
                  })
                  if (statusRes.code === 200 && statusRes.data === 'SUCCESS') {
                    // 支付成功
                    uni.hideLoading()
                    uni.showToast({
                      title: '支付成功',
          ...
          if (orderRes.code === 200) {
            ...
            if (payRes.code === 200) {
              ...
              wx.requestPayment({
                ...
                success: async () => {
                  // 查询订单状态
                  const statusRes: any = await getReplyOrderStatusAPI({
                    orderNumber,
                    questionId: question.value.id,
                  })
                  if (statusRes.code === 200 && statusRes.data === 'SUCCESS') {
...
const handleUserInfoConfirm = async (userInfo: { avatarUrl: string; nickName: string }) => {
  try {
    // 更新用户信息
    const res: any = await updateUserProfileAPI({
      userId: userStore.profile?.userId,
      ...userInfo,
    })
